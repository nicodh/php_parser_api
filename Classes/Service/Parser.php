 ...
 * @version $ID:$
 */
class Tx_Classparser_Service_Parser implements t3lib_singleton {

	/**
	 *
	 * @var Tx_Classparser_Domain_Model_Class_Class
	 */
	...
	protected $currentMethod = Array(
		'reflection' => NULL, // the Tx_Classparser_Reflection_MethodReflection returned from ClassReflection
		'methodObject' => NULL, // the new created Tx_Classparser_Domain_Model_Class_Method
		'endline' => 0,
		'methodBody' => ''
	);
	...
	protected function initClassObject($className) {
		...
		$this->classObject = new Tx_Classparser_Domain_Model_Class($className);

		$this->classReflection = new Tx_Classparser_Reflection_ClassReflection($className);
	...
	protected function onMethodFound($methodMatches) {
		...
		try {
			...
			if ($this->currentMethod['reflection']) {
				$classMethod = new Tx_Classparser_Domain_Model_Class_Method($methodName, $this->currentMethod['reflection']);
				...
				throw new Tx_Classparser_Exception_ParseError(
		...
		foreach ($properties as $propertyName => $propertyValue) {
			try {
				...
				if ($reflectionProperty) {
					...
					$classProperty = new Tx_Classparser_Domain_Model_Class_Property($propertyName);
